/*! ------------------------------------------------
 * Project Name: Atelys - Digital Agency & Personal Portfolio HTML Template
 * Project Description: Stand out and express your uniqueness with Atelys - a vibrant and minimal HTML template for creatives, studios and freelancers. Impress your website visitors with a clean, stylish layout and stunning visuals.
...
function mxdLoader() {
  ...
  // arriving from an in-site link: skip the loader, just fade the page in
  if (document.documentElement.classList.contains("rh-nav-in")) {
  ...
  Promise.all([
    Promise.race([
      ...
      new Promise(resolve => setTimeout(resolve, loaderMaxWait))
    ]),
  ...
  gsap.timeline()
    ...
    .add(() => {
      loader.style.display = "none";
      // slide the black backdrop away to reveal the page (original behaviour)
      gsap.to(pageTransition, { y: "-100%", duration: 0.7, ease: "hop" });
      mxdPageTransition();
      pageAppearance();
    });
  });
...
// page transition — a clean, minimal crossfade on its own overlay.
// Only opacity animates, so layout, fixed elements and ScrollTrigger
// pins are never touched. Uses .rh-fade, NOT .mxd-page-transition
// (that one is the loader's black backdrop).
function mxdPageTransition() {
  const fade = document.querySelector(".rh-fade");
  if (!fade) return;
  ...
  document.querySelectorAll("a[href]").forEach(link => {
    ...
    link.addEventListener("click", e => {
      ...
      try { sessionStorage.setItem("rh-nav-pending", "1"); } catch (err) {}
      gsap.to(fade, {
        opacity: 1,
        duration: 0.22,
        ease: "power2.out",
...
// Entrance half. The overlay is already opaque at first paint (the
// pre-paint head script stamps html.rh-nav-in), so the page is never
// seen mid-layout — this just fades it away.
function mxdPageEnter() {
  const fade = document.querySelector(".rh-fade");
  if (!fade) return;
  gsap.to(fade, {
    opacity: 0,
    duration: 0.3,
    ease: "power2.out",
    onComplete: () => { document.documentElement.classList.remove("rh-nav-in"); }
  });
}
...
$(function() {
  ...
  $("#contact-form").submit(function() {
    ...
    // Endpoint is configured once in js/form-config.js.
    // Falls back to mail.php (only works on a PHP host).
    var endpoint = window.RH_FORM_ENDPOINT || 'mail.php';
    var isRemote = /^https?:\/\//i.test(endpoint);
    ...
    // If nothing can receive the post (no endpoint configured and no PHP host),
    // hand off to the visitor's mail client so the message is never lost.
    function mailtoFallback() {
      var to = window.RH_CONTACT_EMAIL || 'rushanulhaque@gmail.com';
      var d = {};
      $.each(th.serializeArray(), function(_, f) { d[f.name] = f.value; });
      var body = Object.keys(d)
        .filter(function(k) { return k.charAt(0) !== '_' && d[k]; })
        .map(function(k) { return k + ': ' + d[k]; })
        .join('\n');
      window.location.href = 'mailto:' + to +
        '?subject=' + encodeURIComponent('Message from your website') +
    ...
    $.ajax({
      ...
      url: endpoint,
      data: th.serialize(),
      dataType: isRemote ? 'json' : undefined,
      headers: isRemote ? { Accept: 'application/json' } : undefined
    }).done(showSuccess).fail(mailtoFallback);
